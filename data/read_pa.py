...
def get_pa_data(path="../rs_dataset/PA_obj/"):
...
def prepare_pa_data(target_shape, skip_value=1, 
                       padding_approach="zero", leave_index=-1, 
                       leave_method="leave_one_chip", path="../rs_dataset/PA_obj/", 
                       check_filename=False, testing=False, quantification=False):    
    maps_g, wave_g, mapsize_g, filename_g = get_pa_data(path)
    dnp_obj = GetPAData(maps_g, wave_g, mapsize_g, filename_g, padding_approach=padding_approach)
    sers_maps, concentration, label, filename_prepare, wavenumber, mapsize = dnp_obj.forward(target_shape, 
                                                                                               skip_value,
                                                                                               start_wave=750, 
                                                                                               end_wave=1400)
    not_use_index = np.where(concentration != 1)[0]
    if quantification == True:
        not_use_index = np.where(concentration > 1)[0]
    sers_maps = sers_maps[not_use_index]
    concentration = concentration[not_use_index]
    label = label[not_use_index]
    filename_prepare = filename_prepare[not_use_index]
    ...
    print("Training chip index:", np.unique(tr_chip_index, return_counts=True), np.unique(tr_out[2], return_counts=True))
    print("Testing chip index:", np.unique(tt_chip_index, return_counts=True), np.unique(tt_out[2], return_counts=True))    
...
def split_tr_tt(sers_maps, concentration, label, filename, wavenumber, leave_index, leave_method):
    ...
    tr_peak = np.zeros([len(tr_map), 1]) + np.where(wavenumber >= 1330)[0][0]
    tt_peak = np.zeros([len(tt_map), 1]) + np.where(wavenumber >= 1330)[0][0]
